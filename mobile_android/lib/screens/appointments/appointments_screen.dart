      ...
      children: [
        Container(width: 70, height: 70,
          ...
          child: Stack(alignment: Alignment.center, children: [
            const Icon(Icons.calendar_month, color: Colors.black12, size: 36),
            Positioned(bottom: 10, right: 10, child: Container(
              width: 18, height: 18,
              decoration: const BoxDecoration(shape: BoxShape.circle, color: AppTheme.backgroundColor),
              child: const Icon(Icons.error_outline, color: Colors.black38, size: 14),
            )),
          ])),
        const SizedBox(height: 24),
        Text(title, style: const TextStyle(color: Colors.black87, fontSize: 18, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
        const SizedBox(height: 10),
        Text(description, style: const TextStyle(color: Colors.black54, fontSize: 14, height: 1.5), textAlign: TextAlign.center),
      ],
    ));
  }
}
...
class _AppointmentCard extends StatelessWidget {
  ...
  Future<void> _addToGoogleCalendar(BuildContext context) async {
    ...
    final title = 'B&V Barber Randevusu';
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(appointment.serviceName ?? 'Hizmet', style: const TextStyle(color: Colors.black87, fontSize: 16, fontWeight: FontWeight.w600)),
              if (appointment.barberName != null && appointment.barberName!.trim().isNotEmpty)
                Text(appointment.barberName!.trim(), style: const TextStyle(color: Colors.black54, fontSize: 13)),
            ])),
        ...
        Row(children: [
          ...
          Text(dateStr, style: const TextStyle(color: Colors.black87, fontSize: 13)),
        ]),
        const SizedBox(height: 8),
        // Konum
        const Row(children: [
          Icon(Icons.near_me, color: Colors.black54, size: 16),
          SizedBox(width: 8),
          Text('B&V Coffee Barber – Tarsus/Mersin', style: TextStyle(color: Colors.black87, fontSize: 13)),
        ]),
        ...
        if (isUpcoming && appointment.status == AppointmentStatus.confirmed) ...[
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: const Text('Takvime Ekle', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        ...
        // Yorum & Değerlendirme (Geçmiş ve tamamlanmış/onaylanmış randevular için henüz yorumlanmamışsa)
        if (!isUpcoming && (appointment.status == AppointmentStatus.completed || appointment.status == AppointmentStatus.confirmed) && !appointment.isReviewed) ...[
          const SizedBox(height: 12),
          SizedBox(
            width: double.infinity,
            height: 46,
  ...
  void _showCancelDialog(BuildContext context) {
    showDialog(context: context, builder: (_) => AlertDialog(
      backgroundColor: Colors.white,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      title: const Text('Randevuyu İptal Et', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold)),
      content: const Text('Bu randevuyu iptal etmek istediğinize emin misiniz?', style: TextStyle(color: Colors.black54)),
      actions: [
